...
V = rand(NUM_STATES,1)*0.1; % V(s)
P = ones(NUM_STATES, NUM_STATES, 2)/NUM_STATES; % P_sa(s')
P_counts = zeros(NUM_STATES, NUM_STATES, 2);
Reward = zeros(NUM_STATES,1); % R(s)
...
while num_failures<max_failures
 while (consecutive_no_learning_trials < NO_LEARNING_THRESHOLD)
  ...
  [~, action] = max([P(state,:,1)*V, P(state,:,2)*V],[],2);
  ...
  P_counts(state, new_state, action) = P_counts(state, new_state, action) + 1;
  Reward(new_state) = R;
